...
export function AbgleichHome({ cards, statements, companies, payments, receipts }: { cards: CreditCard[]; statements: CardStatement[]; companies: Company[]; payments: Receipt[]; receipts: PaymentReceipt[] }) {
  ...
  return <div className="aus-stack">
    ...
    <section className="aus-section"><h2 className="aus-h2">Monatsabrechnungen der Kreditkarten</h2>{statements.length ? <div className="aus-list">{statements.map((s) => { const card = cards.find((c) => c.id === s.credit_card_id); return <div className="aus-item" key={s.id}><span className="aus-item-main"><strong className="aus-item-title">{card?.label || "Karte"} ·•••• {card?.last4}</strong><span className="aus-item-meta">{card?.payment_channel ? `${PAYMENT_CHANNEL_LABEL[card.payment_channel]} · ` : ""}{s.period_start ? `${formatDate(s.period_start)} – ${formatDate(s.period_end)}` : s.file_name || formatDate(s.created_at)}</span></span><span className="aus-item-side"><span className={`aus-chip ${s.status === "fehler" ? "is-danger" : s.status === "verarbeitet" ? "is-done" : "is-draft"}`}>{s.status === "fehler" ? "Prüfen" : s.status === "verarbeitet" ? "Verarbeitet" : "Neu"}</span><Link className="aus-btn aus-btn-sm aus-btn-primary" href={`/auslagen/abgleich/${s.id}`}>Jetzt Belege zuordnen</Link></span></div>; })}</div> : <div className="aus-empty"><p className="aus-empty-title">Noch keine Monatsabrechnungen</p></div>}</section>
