...
export default async function StatementPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: linkedRows } = linkedIds.length ? await db.from("receipts").select("id, user_id, status, receipt_date, merchant, description, currency, gross_amount, net_amount, vat_amount, vat_rate, gross_amount_eur, payment_method, credit_card_id, file_path, file_mime, file_name, extraction, extraction_error, claim_id, created_at, updated_at").in("id", linkedIds) : { data: [] };
  ...
  return <>
    ...
    <StatementDetail statement={statement} card={card} company={company} transactions={transactions} candidates={candidates} linked={linked} receiptUrls={urls} statementUrl={statementUrl} />
  </>;
}
