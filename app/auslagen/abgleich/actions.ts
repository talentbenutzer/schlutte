...
import { assignReceipt, autoMatchStatement, clearMatch, createCard, deleteStatement, markWithoutReceipt, registerStatement, retryStatement, updateCard, updateTransaction } from "@/lib/data/auslagen-cards";
import { parseAmount } from "@/lib/auslagen/format";

type Result = { ok: boolean; id?: string; count?: number; error?: string; warning?: string };

export async function createCardAction(input: { label: string; last4: string; companyId: string; holderName: string }): Promise<Result> {
...
export async function updateCardAction(id: string, input: { label: string; last4: string; companyId: string; holderName: string; active: boolean }): Promise<Result> {
